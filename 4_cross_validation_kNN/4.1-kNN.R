...
# Using kNN, for any point (x_1,x_2) for which we want an estimate of p*(x_1,x_2), we look
# for the k nearest points to (x_1,x_2) and take an average of the 0s and 1s associated with
# these points. We refer to the set of points used to compute the average as the neighborhood.
...
# are two ways to call this function:
    # 1. We can specify a formula and a data frame. The formula looks like y ~ x_1 + x_2 or y~..
...
# Overtraining is when a machine learning model can make good predictions on the training
# set but cannot generalize well to new data. Overtraining is a reason why we have higher
...
# With k-nearest neighbors (kNN), overtraining is at its worst when we set k = 1. With k = 1,
# the estimate for each (x_1,x_2) in the training set is obtained with just the y corresponding
# to that point.

# Oversmoothing can occur when k is too large and does not permit enough flexibility.

# The goal of cross validation is to estimate quantities such as accuracy or expected MSE
# so we can pick the best set of tuning parameters for any given algorithm, such as the k
# for kNN.
